...
void main(void)
{
    ...
    SerialManager_send(serialMan,
                       bench == TRUE ? "VCU is in bench mode.\n" : "VCU is NOT in bench mode.\n");
    ...
    CanManager *canMan =
        CanManager_new(500, 50, 50, 500, 10, 10, 200000,
    ...
    // ubyte1 pot_DRS_LC = 0; // 0 is for DRS and 1 is for launch control/Auto DRS - CHANGE HERE FOR
    // POT MODE

    ReadyToDriveSound       *rtds = RTDS_new();
    BatteryManagementSystem *bms  = BMS_new(serialMan, BMS_BASE_ADDRESS);
    // 240 Nm
    // MotorController *mcm0 = MotorController_new(serialMan, 0xA0, FORWARD, 2400, 5, 10); //CAN
    // addr, direction, torque limit x10 (100 = 10Nm)
    // 75 Nm
    MotorController *mcm0 =
        MotorController_new(serialMan, 0xA0, FORWARD, 2310, 5,
    ...
    while (1)
    {
        ...
        /*switch (CanManager_getReadStatus(canMan, CAN0_HIPRI))
        {
            ...
            case IO_E_NULL_POINTER: SerialManager_send(serialMan, "IO_E_NULL_POINTER: null pointer
        has been passed to function\n"); break; case IO_E_CAN_FIFO_FULL:
        SerialManager_send(serialMan, "IO_E_CAN_FIFO_FULL: overflow of FIFO buffer\n"); break; case
        IO_E_CAN_WRONG_HANDLE: SerialManager_send(serialMan, "IO_E_CAN_WRONG_HANDLE: invalid handle
        ...
        IO_E_CAN_OLD_DATA: SerialManager_send(serialMan, "IO_E_CAN_OLD_DATA: no data has been
        received\n"); break; default: SerialManager_send(serialMan, "Warning: Unknown CAN read
        status\n"); break;
        }*/
        ...
        if (Sensor_EcoButton.sensorValue == TRUE ||
            (Sensor_RTDButton.sensorValue == FALSE &&
             Sensor_HVILTerminationSense.sensorValue ==
                 FALSE)) // temp make rtd button rtd button in lv
        {
        ...
        {
            if (IO_RTC_GetTimeUS(timestamp_EcoButton) > 10000 &&
                IO_RTC_GetTimeUS(timestamp_EcoButton) < 1000000)
            {
        ...
        TorqueEncoder_calibrationCycle(tps,
                                       &calibrationErrors); // Todo: deal with calibration errors
        ...
        */
        ...
        // CoolingSystem_calculations(cs, MCM_getTemp(mcm0), MCM_getMotorTemp(mcm0),
        // BMS_getHighestCellTemp_degC(bms), &Sensor_HVILTerminationSense);
        // CoolingSystem_enactCooling(cs); //This belongs under outputs but it doesn't really matter
        // for cooling
        ...
        // MCM_setRegenMode(mcm0, REGENMODE_FORMULAE); // TODO: Read regen mode from DCU CAN message
        // - Issue #96
        ...
        SafetyChecker_update(sc, mcm0, bms, tps, bps, &Sensor_HVILTerminationSense,
                             &Sensor_LVBattery);
        ...
        // Task end function for IO Driver - This function needs to be called at the end of every SW
        // cycle
        IO_Driver_TaskEnd();
