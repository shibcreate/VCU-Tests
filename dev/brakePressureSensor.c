...
// This value is used for controlling the brake light and triggering the TPS-BPS implausibility
// fault
...
BrakePressureSensor *BrakePressureSensor_new(void)
{
    ...
    // Max/min values from the datasheet, including inaccuracy (important since our BPS sits
    // slightly below 0.5V but still within range) If voltage exceeds these values, a fault is
    // thrown in safety.c. Accuracy below 100PSI is +/- 0.5% of the full scale span (4V), which is
    // +/- 0.2V
...
void BrakePressureSensor_update(BrakePressureSensor *me, bool bench)
{
    ...
    else
    {
        ...
        // BPS0 only
        me->percent =
            me->bps0_percent; // Note: If we had redundant sensors we would average them here
...
/*-------------------------------------------------------------------
* CalibrateTPS
* Description: Records TPS minimum/maximum voltages (when?) and stores them (where?), or flags that
...
void BrakePressureSensor_calibrationCycle(BrakePressureSensor *me, ubyte1 *errorCount)
{
    ...
    {
        if (IO_RTC_GetTimeUS(me->timestamp_calibrationStart) <
            (ubyte4) (me->calibrationRunTime) * 1000 * 1000)
        {
...
void BrakePressureSensor_getIndividualSensorPercent(BrakePressureSensor *me, ubyte1 sensorNumber,
                                                    float4 *percent)
{
    // Sensor* bps;
    // ubyte2 calMin;
    ...
    // float4 TPS0PedalPercent = getPercent(me->tps0->sensorValue, calMin, calMax, TRUE); //Analog
    // Input 0
}
...
-------------------------------------------------------------------*/
void BrakePressureSensor_getPedalTravel(BrakePressureSensor *me, ubyte1 *errorCount,
                                        float4 *pedalPercent)
{
