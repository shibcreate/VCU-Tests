...
struct _MotorController
{
    ...
    //----------------------------------------------------------------------------
    ubyte2
        canMessageBaseId; // Starting message ID for messages that will come in from this controller
    ...
    // Reverse not allowed
    ubyte2
        torqueMaximumDNm; // Max torque that can be commanded in deciNewton*meters ("100" = 10.0 Nm)

    // Regen torque calculations in whole Nm..?
    RegenMode regen_mode; // Software reading of regen knob position.  Each mode has different regen
                          // behavior (variables below).
    ubyte2 regen_torqueLimitDNm; // Tuneable value.  Regen torque (in Nm) at full regen.  Positive
                                 // value.
    ubyte2 regen_torqueAtZeroPedalDNm; // Tuneable value.  Amount of regen torque (in Nm) to apply
                                       // when both pedals at 0% travel.  Positive value.
    float4 regen_percentBPSForMaxRegen; // Tuneable value.  Amount of brake pedal required for full
                                        // regen. Value between zero and one.
    float4 regen_percentAPPSForCoasting; // Tuneable value.  Amount of accel pedal required to exit
                                         // regen.  Value between zero and one.
    ...
    //----------------------------------------------------------------------------
    void(*setTorque)(MotorController* me, ubyte2 torque); //Will be divided by 10 e.g. pass in 100
    for 10.0 Nm void(*setDirection)(MotorController* me, Direction rotation);
    ...
    //        void(*motorController_setTorque)(MotorController* me, ubyte2 torque); //Will be
    divided by 10 e.g. pass in 100 for 10.0 Nm
...
MotorController *MotorController_new(SerialManager *sm, ubyte2 canMessageBaseID,
                                     Direction initialDirection, sbyte2 torqueMaxInDNm,
                                     sbyte1 minRegenSpeedKPH, sbyte1 regenRampdownStartSpeed)
{
...
void MCM_calculateCommands(MotorController *me, TorqueEncoder *tps, BrakePressureSensor *bps)
{
    //----------------------------------------------------------------------------
    // Control commands
    // Note: Safety checks (torque command limiting) are done EXTERNALLY.  This is a preliminary
    // calculation which should return the intended torque based on pedals Note: All stored torque
    // values should be positive / unsigned
    //----------------------------------------------------------------------------
    ...
    // appsTorque = me->torqueMaximumDNm * getPercent(appsOutputPercent,
    // me->regen_percentAPPSForCoasting, 1, TRUE) - me->regen_torqueAtZeroPedalDNm *
    // getPercent(appsOutputPercent, me->regen_percentAPPSForCoasting, 0, TRUE); bpsTorque = 0 -
    // (me->regen_torqueLimitDNm - me->regen_torqueAtZeroPedalDNm) * getPercent(bps->percent, 0,
    // me->regen_percentBPSForMaxRegen, TRUE);

    /* TESTING PURPOSEs ONLY FOR HARDCODING
    float4 motorRPM = (float4)MCM_getMotorRPM(me);
   float4 kilowatts =  (float4)((float4)MCM_getPower(me)/1000); // divide by 1000 to get watts -->
   kilowatts if(kilowatts > KWH_LIMIT) {

          float4 predictedtq = (float4)(kilowatts*9549.0/motorRPM)*10.0;// (+ val: should be in
   thousands---> read in tens on CAN float4 test =(float4)(KWH_LIMIT*9549.0*10.0); float4 tqsetpoint
   = (float4)(test/motorRPM); // (+ val:)  should be in thousands/high hundreds---> read in tens on
   CAN
    ...
    */
    ...
    {
        torqueOutput = appsTorque + bpsTorque;
        // torqueOutput = me->torqueMaximumDNm * appsOutputPercent;  //REMOVE THIS LINE TO ENABLE
        // REGEN
    }
    // Safety Check. torqueOutput Should never rise above 240. Leaving a +25% buffer in case of
    // rounding errors or comical math
    if (torqueOutput > 300)
    {
        torqueOutput = 0;
    }
...
void MCM_relayControl(MotorController *me, Sensor *HVILTermSense)
{
    ...
    else // HVILTermSense->sensorValue == TRUE || me->HVILOverride == TRUE
    {
        ...
        if (me->previousHVILState == FALSE)
        {
            ...
            } // Reset the startup procedure because HV just went high and we are now turning on the
              // MCM
        }
...
void MCM_inverterControl(MotorController *me, TorqueEncoder *tps, BrakePressureSensor *bps,
                         ReadyToDriveSound *rtds)
{
    ...
    //----------------------------------------------------------------------------
    // New Handshake NOTE: Switches connected to ground.. TRUE = high = off = disconnected = open
    // circuit, FALSE = low = grounded = on = connected = closed circuit
    ...
    case 1: // MCM relay is on, lockout=enabled, inverter=disabled --> stay until lockout is
            // disabled
    ...
    case 2: // MCM on, lockout=disabled, inverter=disabled --> stay until RTD button pressed
        ...
        // How to transition to next state ------------------------------------------------
        if (Sensor_RTDButton.sensorValue == FALSE && tps->calibrated == TRUE &&
            bps->calibrated == TRUE && tps->travelPercent < .05 &&
            bps->percent >
                .25 // Should be high enough to ensure driver is on the brakes reasonably hard
        )
        {
    ...
    case 3: // inverted=disabled, rtd=pressed, waiting for inverter to be enabled
        // Actions to perform upon entering this state
        // ------------------------------------------------ Nothing: wait for mcm to say it's
        // enabled

        // How to transition to next state ------------------------------------------------
        if (MCM_getInverterStatus(me) == ENABLED)
        {
            RTDPercent = 1; // Doesn't matter if button is no longer pressed - RTD light should be
                            // on if car is driveable
            SerialManager_send(me->serialMan,
                               "Inverter has been enabled.  S                                      "
                               "              tarting RTDS.  Car is ready to drive.\n");
            RTDS_setVolume(rtds, 1, 1500000);
            MCM_setStartupStage(me, 4); // MCM_getStartupStage(me) + 1);  //leave this stage since
                                        // we've already kicked off the RTDS
        }
        break;
...
void MCM_parseCanMessage(MotorController *me, IO_CAN_DATA_FRAME *mcmCanMessage)
{
    ...
    case 0x0A8:
        // 0,1 Flux Command
        // 2,3 flux feedback
        // me->ID = ((ubyte2)mcmCanMessage->data[5] << 8 | mcmCanMessage->data[4]) / 10;//4,5 id
        // feedback me->IQ = ((ubyte2)mcmCanMessage->data[7] << 8 | mcmCanMessage->data[6]) /
        // 10;//6,7 iq feedback
        break;
...
void MCM_update_LC_state(MotorController *me, bool newState) { me->launchControlState = newState; }
//----------------------------------------------------PL-------------------------------
...
sbyte2 MCM_get_PL_torqueCommand(MotorController *me) { return me->plTorqueCommand; }

void MCM_set_PL_updateStatus(MotorController *me, bool newState) { me->plActive = newState; }
bool MCM_get_PL_state(MotorController *me) { return me->plActive; }
...
sbyte4 MCM_getMotorRPM(MotorController *me) { return me->motorRPM; }

sbyte4 MCM_getDCVoltage(MotorController *me) { return me->DC_Voltage; }

sbyte4 MCM_getDCCurrent(MotorController *me) { return me->DC_Current; }
