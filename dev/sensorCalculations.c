 ...
 *must be immediately shut down completely. It is not necessary to completely deactivate the
 *tractive system, the motor controller(s) shutting down the power to the motor(s) is sufficient.
 ****************************************************************************/

// Physical pedal travel will only occur across the center (about 1/2) of the actual sensor's range
// of travel The rules (especially EV2.3.6) are written about % of PEDAL travel, not percent of
// sensor range, so we must calculate pedal travel by recording the min/max voltages at min/max
// throttle positions
...
////    IO_RTC_StartTime(&timestamp_calibrationstart);  //Get a timestamp of when this task started
///from the Real Time Clock /                                                    //TODO: Don't start
///coundown until values have been set /    while (IO_RTC_GetTimeUS(timestamp_calibrationstart) <
///(ubyte4)secondsToRunCalibration * 1000 * 1000)   // Give the user # seconds to do this /    {
...
//        //      Once calibration data makes sense, show pedal location (0-10%, 10-90%, 90-100%)
//        with bars
//
...
//
//    //TODO: Should we calculate this during the main loop? (probably not - what if it's not
//    calibrated?)
//    //      or should we only run this when someone specifically asks for throttle position?
//    (probably)
//
...
//    //Note: IC cars may continue to drive for up to 100ms until valid readings are restored, but
//    EVs must immediately cut power
//    //Note: We need to decide how to report errors and how to perform actions when those errors
//    occur.  For now, I'm calling an imaginary Err.Report function
//    //-------------------------------------------------------------------
//    if ((TPS0_Val < TPS0_Min) || (TPS0_Val > TPS0_Max))
//    {
//        //TODO: Err.Report(Err.Codes.TPS0Range, "TPS0 out of range:" & TPS0.sensorValue,
//        Motor.Disable);
//        //Note: We want to continue to run the rest of the code in this function to detect
//        additional errors which we can report back to the dash.  To prevent the rest of the code
//        from applying throttle, we take note that an error has occurred
//        (*errorCount)++;
//    }
//
//    if ((TPS1_Val < TPS1_Min) || (TPS1_Val > TPS1_Max))
//    {
//        //TODO: Err.Report(Err.Codes.TPS1Range, "TPS1 out of range" & Pin140.sensorValue,
//        Motor.Disable);
//        (*errorCount)++;
//
//    }
...
//    float4 TPS0PedalPercent = getPercent(TPS0_Val, TPS0_CalMin, TPS0_CalMax, TRUE); //Analog Input
//    0 float4 TPS1PedalPercent = getPercent(TPS1_Val, TPS1_CalMin, TPS1_CalMax, TRUE); //Analog
//    input 1
//
//    //Check for implausibility (discrepancy > 10%)
//    //RULE: EV2.3.6 Implausibility is defined as a deviation of more than 10% pedal travel between
//    the sensors. if (fabs(TPS1PedalPercent - TPS0PedalPercent) > .1)
//    {
...
void CalibrateBPS(void)
{
    ...
    // Note: It may not be feasible to measure max pressure that each driver will apply.  Instead,
    // it may be best to go with absolute values (.5 to 4.5 V), or to at least record PSI or Voltage
    // at zero pedal.  Not sure how this is affected by having a Sealed sensor (as opposed to Gage),
    // but presumably brake force is SIGNIFICANTLY higher than ambient temperature anyway.
...
void GetBrakePosition(void)
{
    ...
    {
        // Err.Report(Err.Codes.BPSRange, "BPS voltage out of range:" & Sensor_BPS.GetVoltage,
        // Motor.Disable); TPSErrorState++;
    }
