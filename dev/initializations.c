...
void vcu_initializeADC(bool benchMode)
{
    ...
    IO_POWER_Set(IO_SENSOR_SUPPLY_VAR, IO_POWER_14_5_V); // IO_POWER_Set(IO_PIN_269,
                                                         // IO_POWER_8_5_V);
    ...
    IO_PWM_Init(IO_PWM_02, 100, TRUE, FALSE, 0, FALSE,
                NULL); // Pin, Frequency Hz, Boolean for Pos polarity, Current measurement enabled
                       // bool, Weird other pin (current), No diag margin, Not safety Critical
    ...
    IO_PWM_SetDuty(IO_PWM_04, 1.00 * 0xFFFF,
                   NULL); // Default 100%, build up momentum in fans or whatever? Lol
    ...
    // TPS/BPS
    // Sensor_BPS0.ioErr_init = IO_ADC_ChannelInit(IO_ADC_5V_02, IO_ADC_RATIOMETRIC, 0, 0,
    // IO_ADC_SENSOR_SUPPLY_0, NULL);
    if (benchMode == TRUE)
    {
        ...
        Sensor_BPS0.ioErr_signalInit =
            IO_ADC_ChannelInit(IO_ADC_5V_02, IO_ADC_RESISTIVE, 0, 0, 0, NULL);
        Sensor_BPS1.ioErr_signalInit =
            IO_ADC_ChannelInit(IO_ADC_5V_03, IO_ADC_RESISTIVE, 0, 0, 0, NULL);
    }
    else // Not bench mode
    {
        ...
        // Redo BPS ratiometric
        Sensor_TPS0.ioErr_signalInit = IO_ADC_ChannelInit(IO_ADC_5V_00, IO_ADC_RATIOMETRIC, 0, 0,
                                                          IO_ADC_SENSOR_SUPPLY_0, NULL);
        Sensor_TPS1.ioErr_signalInit = IO_ADC_ChannelInit(IO_ADC_5V_01, IO_ADC_RATIOMETRIC, 0, 0,
                                                          IO_ADC_SENSOR_SUPPLY_1, NULL);
        Sensor_BPS0.ioErr_signalInit = IO_ADC_ChannelInit(IO_ADC_5V_02, IO_ADC_RATIOMETRIC, 0, 0,
                                                          IO_ADC_SENSOR_SUPPLY_0, NULL);
        Sensor_BPS1.ioErr_signalInit = IO_ADC_ChannelInit(IO_ADC_5V_03, IO_ADC_RATIOMETRIC, 0, 0,
                                                          IO_ADC_SENSOR_SUPPLY_1, NULL);
    }
    ...
    Sensor_SAS.ioErr_signalInit =
        IO_ADC_ChannelInit(IO_ADC_5V_04, IO_ADC_ABSOLUTE, 0, 0, IO_ADC_SENSOR_SUPPLY_1, NULL);
    ...
    Sensor_DRSKnob.ioErr_signalInit =
        IO_ADC_ChannelInit(IO_ADC_VAR_00, IO_ADC_ABSOLUTE, IO_ADC_RANGE_25V, 0, 0, NULL);
    ...
    Sensor_LCButton.ioErr_signalInit =
        IO_DI_Init(IO_DI_03, IO_DI_PD_10K); // Launch Control Enable Button
...
Sensor Sensor_BPS1;   // = { 2, 0.5, 4.5 }; //Rear brake system pressure (separate address in case
