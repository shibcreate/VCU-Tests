...
void SafetyChecker_update(SafetyChecker *me, MotorController *mcm, BatteryManagementSystem *bms,
                          TorqueEncoder *tps, BrakePressureSensor *bps, Sensor *HVILTermSense,
                          Sensor *LVBattery)
{
    ...
    // EVs must immediately cut power Note: We need to decide how to report errors and how to
    // perform actions when those errors occur.  For now, I'm calling an imaginary Err.Report
    // function

    //-------------------------------------------------------------------
    // Torque Encoder
    //-------------------------------------------------------------------
    if (tps->tps0->sensorValue < tps->tps0->specMin ||
        tps->tps0->sensorValue > tps->tps0->specMax ||
    ...
    // EV2.3.6 Implausibility is defined as a deviation of more than 10 % pedal travel between the
    // sensors.
    //-------------------------------------------------------------------
    ...
    TorqueEncoder_getIndividualSensorPercent(tps, 0,
                                             &tps0Percent); // borrow the pedal percent variable
    ...
    //      b. The Motor shut down must stay active until the APPS signals less than 5% Pedal
    //      Travel, with or without brake operation
    // EV.5.7.2 The Motor shut down must remain active until the APPS signals less than 5% pedal
    // travel, with or without brake operation.
    //-------------------------------------------------------------------
    bool tpsAbove25Percent =
        (tps->travelPercent > .25); // Rules is 25% this is a hack that is made to check
    ...
    //      b. Turn on the AMS Indicator Light (handled by Shutdown circuit)    - Handled by
    //      Shutdown Circuit
    //-------------------------------------------------------------------
    ...
    if ((BMS_getHighestCellVoltage_mV(bms) - BMS_getLowestCellVoltage_mV(bms)) >
        (BMS_MAX_CELL_MISMATCH_V * 1000))
    {
    ...
    else if (IO_RTC_GetTimeUS(timestamp_SoftBSPD) >= 500000 ||
             IO_RTC_GetTimeUS(timestamp_SoftBSPD) == 0)
    {
    ...
    // If over temperature fault detected
    if (BMS_getHighestCellTemp_d_degC(bms) >
        (BMS_MAX_CELL_TEMPERATURE_WARNING * BMS_TEMPERATURE_SCALE))
    {
...
void SafetyChecker_reduceTorque(SafetyChecker *me, MotorController *mcm,
                                BatteryManagementSystem *bms, WheelSpeeds *wss)
{
    float4 multiplier = 1;
    ...
    //////////    tempMultiplier = 1 - getPercent(max(BMS_getPower(bms), MCM_getPower(mcm)), 75000,
    ///80000, TRUE);
    ...
    //////////        if (tempMultiplier < 1) { SerialManager_send(me->serialMan, "SC.Mult: Regen <
    ///15kph\n"); }
    //////////    }
    ////////if (tempMultiplier < multiplier) { multiplier = tempMultiplier; }
    ...
    /* if ( groundSpeedKPH < MCM_getRegenRampdownStartSpeed(mcm))
     ...
     MCM_getRegenRampdownStartSpeed(mcm), TRUE);
         //float4 regenMultiplier = 1 - getPercent(WheelSpeeds_getGroundSpeed(wss),
     MCM_getRegenMinSpeed(mcm), MCM_getRegenRampdownStartSpeed(mcm), TRUE);
             //USE FOR WHEEL SPEEDS TO DETECT RAMP DOWN
         if (regenMultiplier < multiplier ) { multiplier = regenMultiplier; } // Use regenMultiplier
     if it is lower
     }
     */
